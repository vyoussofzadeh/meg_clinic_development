function s_data_dics = do_source_dics(cfg_main, ep_data)
...
if ask==1
    ...
    cfg.kappa = [];
    s_data_dics = do_source_freq(cfg, f_data);
    ...
    %% Positive/negative effects only
    %     cfg = [];
    %     cfg.parameter = 'pow';
    ...
    %     source_diff_dics.dim     = cfg_main.template_grid.dim;
    %     source_diff_dics.inside  = cfg_main.template_grid.inside;
    %     source_diff_dics.pow(source_diff_dics.pow<0)=0; % pos
    %     source_diff_dics.pow(source_diff_dics.pow>0)=0; % neg
    
    %%
    % outputdir_dics = fullfile(outputdir,'dics');
    % if exist(outputdir_dics, 'file') == 0, mkdir(outputdir_dics), end
    toi = cfg_main.toi;
    cfg_main.flag.savetag = 0;
    ...
    if cfg_main.plotting ==1
        ...
        cfg.method        = 'ortho';
        cfg.nslices = 16;
        source_int_dics = do_source_plot(cfg, source_diff_dics);
