function [time_occur, out_val] = do_spikedetection_tfr_chans(cfg, val)
...
while thre_ok == 0
    ...
    %%
    [~, peaksample] = findpeaks(d_in, 'MinPeakDistance', 10); % peaks have to be separated by 300 sample points to be treated as separate
%     [~, peaksample] = findpeaks(d_in, 'MinPeakHeight', 2); % peaks have to be separated by 300 sample points to be treated as separate
    ...
    if cfg.plot == 1
        ...
        figure, imagesc(val.time, val.freq, ts_pp);
