...
cfg.length    = 3;
cfg.overlap   = 0.2;
trm_data_seg = ft_redefinetrial(cfg,trm_data); %trimmed data
...
cfg.savepath = [];
cfg.latency = [trm_data_seg.time{1}(1),trm_data_seg.time{1}(end)];
cfg.rejectpercentage = .95;
cfg.method = 'auto'; % 'manual'
[r_data,report] = do_rejectdata(cfg, trm_data_seg);
...
[cfg, artifact_jump] = ft_artifact_zvalue(cfg,r_data);
...
% [cfg, artifact_jump] = ft_artifact_jump(cfg, r_data)
...
% feedback
cfg.artfctdef.zvalue.interactive = 'yes';
[~, artifact_EOG] = ft_artifact_zvalue(cfg,r_data);

%%
cfg = [];
cfg.artfctdef.reject = 'complete'; %'partial'; %'complete'; %'value'; % this rejects complete trials, use 'partial' if you want to do partial artifact rejection
% cfg.continuous = 'yes';
cfg.artfctdef.eog.artifact = artifact_EOG; %
cfg.artfctdef.jump.artifact = artifact_jump;
% cfg.artfctdef.value = 0;
% cfg.artfctdef.muscle.artifact = artifact_muscle;
trm_nar = ft_rejectartifact(cfg,r_data);
...
cfg.blocksize   = 3;         % view the continous data in 30-s blocks
