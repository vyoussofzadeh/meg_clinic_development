...
%% Visual artifact
% cfg_visu = [];
% cfg_visu.viewmode = 'vertical';
% cfg_visu.plotevents = 'no';
% cfg_visu.selectmode = 'markartifact';
% cfg_visu.blocksize   = cln_data.time{1}(end);
% cfg_visu.continuous='yes';
% cfg_art = ft_databrowser(cfg_visu,trm_data);
% 
% 
% cfg = [];
...
% % cfg.continuous = 'yes';
% cfg.artfctdef.visual.artifact = cfg_art.artfctdef.visual.artifact; %
...
% [cfg, artifact_ECG]           = ft_artifact_ecg(cfg, trm_data);
...
aft = [];
aft.eog = artifact_EOG;
aft.jump = artifact_jump;

%%
cfg = [];
cfg.channel = modal;
% cln_data = ft_selectdata(cfg,trm_nar);
...
% cfg.length    = 1;
% cfg.overlap   = 0;
% cln_data_seg = ft_redefinetrial(cfg,cln_data); %trimmed data
% 
% %%
% m = [];
% for i=1:length(cln_data_seg.trial)
%     m(i) = mean(cln_data_seg.trial{i}(1,:)); 
% end
% % idx_badtrial = find(isnan(m)==1);
% 
% %
% cfg = [];
% cfg.trials = find(isnan(m)~=1);
% cln_data_seg_sel = ft_selectdata(cfg, cln_data_seg);
...
% cfg.blocksize   = 120;         % view the continous data in 30-s blocks
% ft_databrowser(cfg, cln_data);


% cfg = [];
% cfg.channel = modal;
% cln_data_no = ft_selectdata(cfg,trm_data);
% cfg             = [];
% cfg.continuous  = 'yes';
% cfg.viewmode    = 'vertical'; % all channels seperate
% cfg.blocksize   = 30;         % view the continous data in 30-s blocks
% ft_databrowser(cfg, cln_data_no);
...
while flag.analysis == 1
    ...
    % askplot = input(': ');
    askplot =1;
    cfg = []; cfg.plot = askplot; cfg.art = aft; 
    switch ask.freq_occur_sel       
        case 1
        ...
        case 2
            cfg.foi = [1,5]; cfg.ttl = [modal, ' slow-rate'];
        ...
        case 3
            cfg.foi = [25,40]; cfg.ttl = [modal, 'high-rate'];
        ...
        case 4
            cfg.foi = [foi(1),foi(2)]; cfg.ttl = [modal, ' selected-rate'];
...
%     cfg.continuous  = 'yes';
%     cfg.viewmode    = 'vertical'; % all channels seperate
%     cfg.blocksize   = cln_data.time{1}(end);         % view the continous data in 30-s blocks
% %     cfg.plotlabels              = 'yes'; 
% %     cfg.ploteventlabels         = time_occur
% %     cfg.plotevents              = 'yes';
%     cfg.selectmode              =  'markpeakevent'; %'markartifact'
%     ft_databrowser(cfg, cln_data);
    
    
%     for j=1:length(time_occur)
%         hold on
%         plot([time_occur(j) time_occur(j)],[1, 10],'-', 'Color',[0.7 0.7 0.7]);
